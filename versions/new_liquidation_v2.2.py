...
SCENARIOS = {
    "value-original": {
        ...
        "output_dir": "output\\value\\strategy-original\\value2",
    },
    "value-final": {
        # "invest_json": "data\\value_filter4_modified_with_sharpe_10_drop_20_iter.json",
        "invest_json": "data\\value_filter4_modified_with_sharpe_10_drop_20_iter_4_out_of_5.json",
        "good_json": "data\\filtered_outputs\\Value_Filter_2_50%.json",
        "output_dir": "output\\value\\strategy-final2\\value2",
    },
    "momentum-original": {
        "invest_json": "data\\momentum.json",
        "good_json": "data\\good_stocks_combined.json",
        "output_dir": "output\\momentum\\strategy-original\\momentum2",
    },
    "momentum-final": {
        "invest_json": "data\\momentum_modified_filter4_with_sharpe_10_drop_20_iter.json",
        "good_json": "data\\good_stocks_combined.json",
        "output_dir": "output\\momentum\\strategy-final\\momentum2",
    },
}
        ...
        if i >= 1 and portfolio_value_before > 0 and sellable_lots:
            ...
            if total_possible_liquidation <= allowed_liquidation_limit + 1e-9:
                ...
                if not PARTIAL_ALLOWED:
                    ...
                    # Sell up to allowed_liquidation_limit in worst-stock-first order
                    remaining_to_liquidate = allowed_liquidation_limit
                    # Compute stock-level return to sort worst-first
                    stock_returns = {}
                    for t in sellable_lots.keys():
                        # use holding_summaries if available
                        if t in holding_summaries and holding_summaries[t]["avg"]:
                            avg_buy = holding_summaries[t]["avg"]
                        else:
                            # fallback: compute weighted avg from lots
                            lots = holdings.get(t, {}).get("lots", [])
                            invested = sum(l["qty"] * l["price"] for l in lots)
                            qty = sum(l["qty"] for l in lots)
                            avg_buy = safe_div(invested, qty)
                        # current price cached
                        cache_key = (t, str(liq_day))
                        cur_price = price_cache.get(
                            cache_key, get_price(df, t, liq_day)
                        )
                        stock_returns[t] = safe_div(cur_price, avg_buy) - 1.0

                    # Sort stocks by worst return (lowest) first
                    stocks_sorted = sorted(
                        sellable_lots.keys(), key=lambda x: stock_returns.get(x, 0.0)
                    )

                    for t in stocks_sorted:
                        if remaining_to_liquidate <= 1e-9:
                            break
                        # For each sellable lot in FIFO order for this stock
                        # We will attempt to sell each lot completely; if the lot value exceeds remaining_to_liquidate,
                        # we will sell a partial quantity from that lot to meet the exact remaining amount.
                        for lot in sellable_lots[t]:
                            if remaining_to_liquidate <= 1e-9:
                                break
                            price = lot["current_price"]
                            total_lot_value = lot["qty"] * price

                            if total_lot_value <= remaining_to_liquidate + 1e-9:
                                # sell entire lot
                                qty_to_sell = lot["qty"]
                            ...
                            ev = {
                                ...
                                "Liquidated_Amount": round(amount, 2),
                                "Reason": "Lot-sellable (partial mode)",
                                "Remaining_Qty": round(
                                    holdings.get(t, {}).get("qty", 0.0), 6
                                ),
                                "Portfolio_Value_After": None,
